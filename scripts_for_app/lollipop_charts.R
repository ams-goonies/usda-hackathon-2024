...
demo <- readRDS('data/finalized/ALL_DATA.rds') %>%
  filter(metric == 'Number of producers',
         county_name == 'All')
#reduced_commodities_by_state <- read.csv('C:/Users/Stella.Koh/OneDrive - USDA/2022 Ag Census/reduced_commodity_pct_by_state.csv')
reduced_commodities_by_state <- readRDS('data/finalized/ALL_DATA.rds') %>%
  filter(metric == 'Total sales, $',
         county_name == 'All'
         )
...
lollipop_chart <- function(variable = "commodities", state_input = NA, size_input = "All", n = 10) {
  ...
  if (variable == "commodities") { 
    ...
    lollipop <- ggplot(filtered) +
      geom_segment(
        aes(
          x=reorder(short_desc, Value_2022), 
          xend=short_desc, 
          y=Value_2017, 
          yend=Value_2022, 
          color = change_pct#,
    ...
    lollipop <- ggplot(filtered) +
      geom_segment(
        aes(
          x=factor(short_desc, levels = rev(all)), 
          xend=short_desc, 
          y=Value_2017, 
          yend=Value_2022, 
          color = change_pct#,
  ...
  ggplotly(lollipop, tooltip = c("text")) +
